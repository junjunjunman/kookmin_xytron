...
def three_mission(cv_image):
    ...
    # 2. 사용자가 마우스로 획득한 픽셀 좌표 기반 ROI 지정
    # Numpy Array 슬라이싱 구조: [Y_시작:Y_끝, X_시작:X_끝]
    roi = cv_image[87:144, 336:393]

    # 3. 조명 변화에 강건한 HSV 색 공간 변환
    hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)

    # 4. 초록색(파란불) 불빛의 HSV 검출 범위 정의
    lower_green = np.array([35, 100, 100])
    upper_green = np.array([85, 255, 255])

    # 5. 범위 안의 픽셀만 걸러내는 마스크 생성 및 개수 카운트
